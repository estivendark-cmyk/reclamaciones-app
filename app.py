...
st.set_page_config(page_title="Gestor de Reclamaciones & SIMIT", layout="wide")
...
with st.sidebar.expander("📊 Reporte de Reclamaciones"):
    valor_recla = st.text_input("MONTO RECLAMACIÓN", value="$ 5.467.111,00")
    estado_siniestro = st.selectbox("Estado", ["PAGADO", "EN PROCESO", "OBJETADO", "VIGENTE"])
...
with st.sidebar.expander("🚦 Información SIMIT (Multas)", expanded=True):
    texto_simit = st.text_area("Pega aquí el estado de cuenta del SIMIT:", 
                                placeholder="Ejemplo: No tiene multas pendientes / Comparendo N°... por valor de...")

# --- SECCIÓN ANEXOS ---
with st.sidebar.expander("📄 Otros Detalles y Texto Pegado"):
    texto_anexo = st.text_area("Pega aquí cualquier otra información adicional:")

# --- CUERPO PRINCIPAL ---
st.markdown("<h2 style='color: #001e4d; text-align: center;'>📋 Sistema de Soporte e Historial SIMIT</h2>", unsafe_allow_html=True)

# Herramientas Rápidas (RUNT, FASECOLDA, SIMIT)
...
uploaded_file = st.file_uploader("📸 Sube la evidencia (Pantallazo RUNT/SIMIT o Foto)", type=["jpg", "png", "jpeg"])
    ...
    if st.button("🚀 GENERAR REPORTE INTEGRAL"):
        ...
        <body>
            <div class="header">
                ...
                <p style="margin:5px 0 0 0;">Validación de Vehículo, Siniestros y Multas (SIMIT)</p>
            </div>
            <div class="yellow-banner">DOCUMENTO PARA FINES DE CONSULTA E INFORMACIÓN GENERAL</div>

            <div class="container">
                <div style="display: flex; gap: 15px;">
                    <div style="width: 50%;">
                        <div class="card">
                            <div class="card-header">DATOS DEL VEHÍCULO Y ASEGURADO</div>
                            <table class="data-table">
                                <tr><td style="font-weight:bold;">PLACA:</td><td>{placa}</td></tr>
                                <tr><td style="font-weight:bold;">CLIENTE:</td><td>{nombre_asegurado}</td></tr>
                                <tr><td style="font-weight:bold;">MODELO:</td><td>{marca_linea} ({modelo})</td></tr>
                            </table>
                        </div>
                        <div class="card">
                    ...
                    <div style="width: 50%;">
                        <div class="card" style="height: 100%;">
                            <div class="card-header">EVIDENCIA ADJUNTA</div>
                ...
                <div class="card">
                    <div class="card-header">ESTADO DE CUENTA SIMIT (MULTAS Y COMPARENDOS)</div>
                    <div class="text-box">
                        {texto_simit if texto_simit else "No se registraron novedades en SIMIT."}
                    </div>
                </div>
            ...
            <div class="footer">
                Información transcrita de fuentes oficiales. Este documento no reemplaza los certificados originales.
            </div>
        </body>
        </html>
        """
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix=".pdf") as tmp:
            HTML(string=html_template).write_pdf(tmp.name)
            with open(tmp.name, "rb") as f:
                st.download_button("📥 DESCARGAR REPORTE INTEGRAL", f, f"Reporte_Simit_{placa}.pdf")
        os.unlink(tmp.name)
